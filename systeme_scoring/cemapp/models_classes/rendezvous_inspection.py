...
class RendezvousInspection(models.Model):
    ...
    def envoyer_notification(self):
        subject = f"Rendez-vous d'inspection pour votre demande de crédit N° : {self.demande.numero_credit}"
        html_message = render_to_string('emails/notification_inspection.html', {
            'rendezvous': self,
            'modification_url': f"{settings.BASE_URL}/proposer-date/{self.token}/"
        })
